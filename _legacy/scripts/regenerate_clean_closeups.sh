#!/bin/bash
# Regenerate closeup scripts without text labels

cd /Users/ishaangubbala/Documents/p53

cat > reports/pymol_scripts/closeup_R175H_wildtype.pml << 'EOF'
# R175H Wildtype DNA Binding
set ray_trace_mode, 1
set ray_shadows, 1
set antialias, 2
set ambient, 0.5
set specular, 0.6
set depth_cue, 1
set bg_rgb, [1, 1, 1]
set cartoon_fancy_helices, 1

load Data/raw/alphafold/AF-P04637-F1-model_v6.pdb, protein
load Data/raw/experimental_pdbs/2OCJ_full.pdb, dna
align dna, protein
hide everything

show cartoon, protein
color gray70, protein
show cartoon, dna
color marine, dna
set cartoon_ring_mode, 3, dna

select mut_site, protein and resi 175
show spheres, mut_site
color cyan, mut_site
set sphere_scale, 1.8, mut_site

select interface, protein and resi 175+241+248+273+280
show sticks, interface
color orange, interface
set stick_radius, 0.35, interface

distance hbonds, interface, dna, 3.5
hide labels, hbonds
color lime, hbonds
set dash_width, 4, hbonds
set dash_gap, 0.1, hbonds

select pocket, protein and (interface around 10)
show surface, pocket
set transparency, 0.6, pocket
color palegreen, pocket

select dna_contact, dna and (interface around 5)
show sticks, dna_contact
color yellow, dna_contact and elem C
set stick_radius, 0.25, dna_contact

center interface
orient interface
zoom interface, 4
turn y, 20
turn x, 10

viewport 1920, 1080
ray 1920, 1080
png reports/figures/dna_binding_closeups/R175H_wildtype_binding.png, dpi=300
print('Image saved!')
EOF

echo "Created closeup scripts (sample shown). Run python script for all 9..."
